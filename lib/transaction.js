...
class DgraphTransaction {
  ...
  async mutate(mutation, options) {
    ...
    if (mutation.set) {
      if (isString(mutation.set)) mut.setSetNquads(mutation.set);else mut.setSetJson(mutation.set);
    }
    if (mutation.del) {
      if (isString(mutation.del)) mut.setDelNquads(mutation.del);else mut.setDeleteJson(mutation.del);
    }
